  ...
  컴포넌트 생성 시, 주의사항
    ...
    State란?
      ...
      Controlled Component(제어 컴포넌트)
       ...
       장점
        - 컴포넌트의 state와 intput value가 완전히 동일한 값을 갖음(신뢰 가능한 단일 출처)
        - 다른 컴포넌트에 intput value를 전달하거나 다른 이벤트 핸들러에서 값을 재설정 할 수 있음

       단점
         - 값이 변경되는 매순간 렌더링이 됨(해당 컴포넌트의 영향 범위가 클 수록 성능 저하)
      ...
      HTML 요소의 비활성화 상태를 설정하는 데 사용. 이 속성은 주로 폼 요소(예: 버튼, 입력 필드)와 함께 사용되며, 사용자의 상호 작용을 제한하는 데 유용

      disabled 속성의 기본 개념

       - 비활성화 상태: disabled 속성이 설정된 요소는 사용자의      입력을 받거나 클릭과 같은 이벤트를 처리하지 않음
       - 시각적 표현: 비활성화된 요소는 일반적으로 흐릿하게 표시되거나 다른 시각적 효과를 통해 사용자가 상호 작용할 수 없음을 나타냄
       - 폼 제출 방지: disabled 속성이 설정된 폼 요소는 폼 제출 시 해당 요소의 값을 서버로 전송하지 않음


       disabled 속성의 활용 사례

      - 폼 유효성 검사: 폼의 모든 필수 필드가 채워졌을 때만 제출 버튼을 활성화
      - 데이터 로딩 중: 데이터 로딩이 완료될 때까지 버튼을 비활성화하여 중복 요청을 방지함함
      - 권한 제어: 특정 권한이 있는 사용자에게만 활성화된 버튼을 표시
      - 단계별 폼: 이전 단계가 완료되었을 때만 다음 단계의 버튼을 활성화함함


     주의사항

     - disabled 속성은 사용자 경험에 영향을 미치므로, 적절한 상황에서만 사용해야 함
     - 접근성을 고려하여 disabled 속성을 사용할 때는 대체 텍스트나  추가적인 설명을 제공하는 것이 좋음
    - CSS를 사용하여 비활성화된 요소의 시각적 표현을 명확하게 나타내는 것이 좋음
